...
class VirtuCamProvider : ContentProvider() {
    ...
    override fun query(
        ...
        return when (uriMatcher.match(uri)) {
            CODE_CONFIG -> {
                MatrixCursor(arrayOf("enabled", "media_uri", "is_video", "is_stream", "stream_url", "target_apps", "compensation_factor", "is_mirrored", "zoom_factor", "rtsp_use_tcp", "rotation", "is_color_swapped", "is_liveness_enabled", "is_test_pattern_mode", "is_passthrough_mode", "rotation_offset", "is_buffer_capture_enabled", "is_color_flash_enabled", "is_refine_mode", "is_rppg_enabled", "rppg_bpm")).apply {
                    addRow(arrayOf(
                        ...
                        if (config.isBufferCaptureEnabled) 1 else 0,
                        if (config.isColorFlashEnabled) 1 else 0,
                        if (config.isRefineMode) 1 else 0,
                        if (config.isRppgEnabled) 1 else 0,
                        config.rppgBpm
                    ))
                }
            }
